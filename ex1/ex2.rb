...
def player_one()
	...
	counter = 0
	$p1_question_array = []
	puts 'Okay, ' + $player_names[0].capitalize + ', what personal question would you like Player 2 to try to answer?'
	question = gets.chomp
	$p1_question_array.push(question)
	puts 'And what is the correct answer?'
	answer = gets.chomp
	$p1_answer_array.push(answer)
	while true
		$question_count = $p1_question_array.length + 1
		random_feedback = rand($question_feedback.length)
		puts $question_feedback[random_feedback] + ' If you\'re all done writing questions, press enter. Otherwise, add another personal question for Player 2 to try to answer. (Question ' + $question_count.to_s + ")"
		question = gets.chomp
...
def player_two()
	...
	counter_two = 0
	points = 0
	while counter_two <= $p1_question_array.length
